...
PromedioDuracionParadas = mean(valores(valores > 0 & valores <= 90), 'omitnan');
                ...
                nuevaFila = table(string(bus), string(fecha), k, id, string(sexo), hora_inicio, hora_final, acelepercent1, acelepercent2, frepercent1,frepercent2, ...
                    ...
                    mean(cell2mat(curva_kalman)), Paradas, PorcentajeParadas,PromedioDuracionParadas,...
...
function exportarColumnasPorIndice(tabla, indicesColumnas, nombreArchivo)
    ...
    if any(indicesColumnas < 1 | indicesColumnas > width(tabla))
        error('Índices fuera de rango.');
    end

    % Seleccionar columnas
    tablaReducida = tabla(:, indicesColumnas);
