        ...
        for j = 1:numel(fechas)
            ...
            if isfield(datosBuses.(bus).(fecha), 'tiempoRuta') && isfield(datosBuses.(bus).(fecha), 'datosSensor')
                ...
                for k = 1:size(tiempoRuta, 1)
                    ...
                    tiempoVelocidad = {inicioIda, finVuelta, velocidadesRuta, ruta};
                ...
                for j = 1:numel(fechas)
                    ...
                    if isfield(datosBuses.(bus).(fecha), 'tiempoRuta') && isfield(datosBuses.(bus).(fecha), 'datosSensor')
                        ...
                        for k = 1:size(tiempoRuta, 1)
                            ...
                            % Concatenar los datos de ida y vuelta
                            datosRuta = [datosIda; datosVuelta];

                            % Almacenar los datos del sensor extraídos
                            tiempoDatosSensor = {inicioIda, finVuelta, datosRuta, ruta};
